...
class viewpatient(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context = super().get_context_data(**kwargs)
        #context['instance_id'] = self.object.id  # Pass the instance_id to the template
        context['instance_id'] = self.object.pk  # Pass the instance_id to the template
        context.update({
...
def updatemanagement(request, id):
    k = Management.objects.get(id=id)
    management =ManagementForm(instance=k)
    if request.method == "POST":
        management = ManagementForm(request.POST, files=request.FILES, instance=k)
        if management.is_valid():
            management.save()
            messages.info(request, 'Updated succesefull.')
            return redirect('patient')
    context = {"management":management}
    return render(request, 'sims/updatemanagement.html', context)
...
def update_clinics_view(request, instance_id):
    # Retrieve the instance you want to update
    instance_to_update = get_object_or_404(Clinics, id=instance_id)

    if request.method == 'POST':
        # Create a form instance and populate it with data from the request
        formclinics = ClinicsForm(request.POST)

        # Check if the form is valid
        if formclinics.is_valid():
            # Update the instance with the form data
            # instance_to_update.Title = formclinics.cleaned_data['Title']
            instance_to_update.Clinics = formclinics.cleaned_data['Clinics']
        ...
        formclinics = ClinicsForm(instance=instance_to_update)

    return render(request, 'sims/update_clinics_view.html', {'formclinics': formclinics})
